...
const admin = { id: "admin", email: "admin@admin.com", fullName: "Admin User", authorities: ["course:create", "course:update", "course:delete", "class:create", "class:update", "class:delete", "class:enrollment:create", "class:enrollment:update", "class:enrollment:delete"] };
...
beforeEach(() => {
  ...
  api.loadClassDetail.mockResolvedValue({ classInfo: courseClass, students: [{ id: "student-1", enrollmentId: "enrollment-1", fullName: "Deniz Arslan", email: "deniz@example.com", phoneMasked: "••• ••• •• ••", enrollmentStatus: "ACTIVE", registrationFee: 12500, paymentPlan: "CASH", installmentCount: null, firstPaymentDate: null, paymentStatus: "PENDING", expectedPaymentDate: "2026-08-20", note: null, payments: [{ id: "payment-1", installmentNumber: 1, installmentTotal: 1, amount: 12500, dueDate: "2026-08-20", status: "PENDING", paidAt: null, paymentMethod: null, version: 0 }], version: 0 }] });
...
describe("CoursesWorkspace", () => {
  ...
  it("sınıf satırından detay ve kayıtlı öğrencileri açar", async () => {
    ...
    expect(await screen.findByText("Ödeme tahsilatı")).toBeInTheDocument();
    expect(screen.getByLabelText("Tahsilat tarihi")).toBeInTheDocument();
    expect(screen.getByLabelText("Tahsilat tipi")).toBeInTheDocument();
    expect(screen.getByText("Nakit")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "Ödemeyi tamamla" }));
    expect(api.receivePayment).toHaveBeenCalledWith(courseClass.id, "enrollment-1", "payment-1", { version: 0, paidAt: expect.any(String), paymentMethod: "CASH" });
    expect((await screen.findAllByText("Ödeme tamamlandı")).length).toBeGreaterThan(0);
    expect(await screen.findByText("Muhasebe Uzmani")).toBeInTheDocument();
  }, 30000);
