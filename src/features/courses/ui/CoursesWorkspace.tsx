...
import { App, Button, DatePicker, Descriptions, Empty, Flex, Form, Grid, Input, InputNumber, Modal, Pagination, Progress, Radio, Segmented, Select, Space, Spin, Table, Tabs, Tag, TimePicker, Tooltip, Typography } from "antd";
...
type ClassFormValues = Omit<CreateClassRequest, "startDate" | "endDate" | "startTime" | "endTime"> & { dateRange: [Dayjs, Dayjs]; timeRange: [Dayjs, Dayjs] };
type EditingRecord = { type: "course"; value: Course } | { type: "class"; value: CourseClass };
type PromissoryNoteFields = {
  promissoryDebtorName?: string;
  promissoryGuarantorName?: string;
  promissoryIssuePlace?: string;
  promissorySerialStart?: string;
};
type EnrollmentFormValues = Omit<CreateClassEnrollmentRequest, "firstPaymentDate" | "expectedPaymentDate" | "paymentPlan"> & PromissoryNoteFields & {
...
const moneyInputFormatter = (value?: string | number): string => {
  const raw = String(value ?? "");
  if (raw === "") return "";
  const [integer, decimal] = raw.split(".");
  return `${integer.replace(/\B(?=(\d{3})+(?!\d))/g, ".")}${decimal === undefined ? "" : `,${decimal}`}`;
};
const moneyInputParser = (value?: string): string => value?.replace(/\./g, "").replace(",", ".") ?? "";
...
export function CoursesWorkspace({ user }: { user: AuthenticatedUser }): JSX.Element {
  ...
  const openEditCourse = (course: Course) => { setEditing({ type: "course", value: course }); form.setFieldsValue({ name: course.name, durationHours: course.durationHours, listPrice: course.listPrice, status: course.status }); setFormOpen(true); };
  const openEditClass = (item: CourseClass) => { setEditing({ type: "class", value: item }); form.setFieldsValue({ name: item.name, courseId: item.courseId, instructorName: item.instructorName, capacity: item.capacity, status: item.status, dateRange: [dayjs(item.startDate), dayjs(item.endDate)], timeRange: [dayjs(item.startTime, "HH:mm:ss"), dayjs(item.endTime, "HH:mm:ss")] }); setFormOpen(true); };
  ...
  const submit = async () => {
    ...
    try {
      if (tab === "courses") {
        ...
        const request = { name: value.name, courseId: value.courseId, instructorName: value.instructorName, capacity: value.capacity, status: value.status, startDate: value.dateRange[0].format("YYYY-MM-DD"), endDate: value.dateRange[1].format("YYYY-MM-DD"), startTime: value.timeRange[0].format("HH:mm"), endTime: value.timeRange[1].format("HH:mm") };
...
function Records(props: RecordsProps): JSX.Element {
  ...
  const courseColumns: ColumnsType<Course> = [
    { title: "Kurs", dataIndex: "name", render: (value) => <div className="courses__primary-cell"><strong>{value}</strong></div> },
  ...
  const classColumns: ColumnsType<CourseClass> = [
    ...
    { title: "Tarih aralığı", dataIndex: "startDate", render: (value, record) => <div className="courses__primary-cell"><span>{formatApiDate(value)}</span><small>{formatApiDate(record.endDate)} tarihine kadar · {formatClassTime(record)}</small></div> },
...
function PaymentDetailsModal({ classId, student, canReceivePayment, onStudentChange, onClose }: { classId: string; student?: EnrolledStudent; canReceivePayment: boolean; onStudentChange: (student: EnrolledStudent) => void; onClose: () => void }): JSX.Element {
  ...
  return <>
  <Modal open={Boolean(student)} title={student ? `${student.fullName} · Ödeme detayları` : "Ödeme detayları"} width={1120} onCancel={onClose} footer={<Button onClick={onClose}>Kapat</Button>} destroyOnHidden>
    {student && <div className="courses__payment-details"><Descriptions bordered size="small" column={{ xs: 1, sm: 2 }} items={[{ key: "fee", label: "Kayıt ücreti", children: currency.format(student.registrationFee) }, { key: "plan", label: "Ödeme türü", children: paymentPlanSummary(student) }, { key: "status", label: "Genel ödeme durumu", children: <PaymentStatusTag status={student.paymentStatus} /> }, { key: "paid", label: "Ödenen", children: currency.format(paidAmount) }, { key: "pending", label: "Bekleyen", children: currency.format(pendingAmount) }, { key: "start", label: student.paymentPlan === "PROMISSORY_NOTE" ? "İlk senet vadesi" : student.paymentPlan === "INSTALLMENT" ? "İlk taksit tarihi" : "Tahmini ödeme tarihi", children: formatOptionalApiDate(student.paymentPlan === "CASH" ? student.expectedPaymentDate : student.firstPaymentDate) }, { key: "note", label: "Not", span: 2, children: student.note || "-" }]} />
      <section aria-labelledby="payment-installments-title"><Typography.Title id="payment-installments-title" level={5}>Ödeme ve taksit planı</Typography.Title><Table rowKey="id" size="small" pagination={false} dataSource={student.payments} scroll={{ x: 1060 }} locale={{ emptyText: <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Ödeme planı bulunmuyor." /> }} columns={[{ title: "Sıra", width: 64, render: (_, payment) => `${payment.installmentNumber}/${payment.installmentTotal}` }, { title: "Vade tarihi", dataIndex: "dueDate", width: 150, render: (value) => formatOptionalApiDate(value) }, { title: "Tutar", dataIndex: "amount", width: 110, align: "right", render: (value) => currency.format(value) }, { title: "Durum", dataIndex: "status", width: 190, render: (value: PaymentStatus) => <PaymentStatusTag status={value} /> }, { title: "Ödeme tarihi", dataIndex: "paidAt", width: 150, render: (value) => formatOptionalApiDate(value) }, { title: "Tahsilat tipi", dataIndex: "paymentMethod", width: 130, render: (value: PaymentMethod | null | undefined) => value ? paymentMethodLabels[value] : "-" }, { title: "Ödemeyi alan", width: 150, render: (_, payment) => payment.status === "COMPLETED" ? optionalText(payment.receivedByFullName) : "-" }, { title: "İşlem", width: 116, render: (_, payment) => payment.status === "PENDING" && canReceivePayment ? <Button size="small" type="primary" loading={receivingPaymentId === payment.id} disabled={Boolean(receivingPaymentId)} onClick={() => openReceipt(payment)}>Ödeme alındı</Button> : "-" }]} /></section>
    </div>}
  </Modal>
...
function ClassDetailModal({ open, startInEnrollment, loading, error, detail, canEnroll, canUpdateEnrollment, canDeleteEnrollment, onDetailChange, onClose, onRetry }: { open: boolean; startInEnrollment: boolean; loading: boolean; error: boolean; detail?: ClassDetail; canEnroll: boolean; canUpdateEnrollment: boolean; canDeleteEnrollment: boolean; onDetailChange: (detail: ClassDetail) => void; onClose: () => void; onRetry: () => void }): JSX.Element {
  ...
  useEffect(() => {
    if (isScheduledPaymentPlan(paymentPlan) && paymentStatus !== "PENDING") {
      enrollmentForm.setFieldValue("paymentStatus", "PENDING");
    }
  }, [enrollmentForm, paymentPlan, paymentStatus]);

  useEffect(() => {
    if (isScheduledPaymentPlan(editPaymentPlan) && editPaymentStatus !== "PENDING") {
      editForm.setFieldValue("paymentStatus", "PENDING");
    }
  }, [editForm, editPaymentPlan, editPaymentStatus]);
  ...
  const saveEnrollments = async () => {
    ...
    try {
      const request: CreateClassEnrollmentRequest = {
        ...
        installmentCount: values.paymentPlan === "INSTALLMENT" || values.paymentPlan === "PROMISSORY_NOTE" ? values.installmentCount : null,
        firstPaymentDate: values.paymentPlan === "INSTALLMENT" || values.paymentPlan === "PROMISSORY_NOTE" ? values.firstPaymentDate?.format("YYYY-MM-DD") ?? null : null,
        ...
        note: mergeEnrollmentNote(values),
      };
  ...
  const saveEnrollmentEdit = async () => {
    ...
    try {
      const updated = await updateEnrollment(detail.classInfo.id, editingEnrollment.enrollmentId, {
        ...
        firstPaymentDate: values.paymentPlan === "INSTALLMENT" || values.paymentPlan === "PROMISSORY_NOTE" ? values.firstPaymentDate?.format("YYYY-MM-DD") ?? null : null,
        expectedPaymentDate: values.paymentPlan === "CASH" && values.paymentStatus === "PENDING" ? values.expectedPaymentDate?.format("YYYY-MM-DD") ?? null : null,
        note: mergeEnrollmentNote(values), version: values.version,
      });
...
function EnrollmentEditForm({ form, paymentPlan, paymentStatus }: { form: ReturnType<typeof Form.useForm<EnrollmentEditFormValues>>[0]; paymentPlan?: PaymentPlanType; paymentStatus?: UpdateClassEnrollmentRequest["paymentStatus"] }): JSX.Element {
  return <Form form={form} layout="vertical" requiredMark="optional">
    <Form.Item name="registrationFee" label="Kayıt ücreti" rules={[{ required: true, message: "Kayıt ücretini yazın." }]}><InputNumber className="courses__money-input" min={0} max={9999999999.99} precision={2} prefix="₺" formatter={moneyInputFormatter} parser={moneyInputParser} /></Form.Item>
    <Form.Item name="paymentPlan" label="Ödeme şekli" rules={[{ required: true }]}><Segmented block options={paymentPlanOptions} /></Form.Item>
    {(paymentPlan === "INSTALLMENT" || paymentPlan === "PROMISSORY_NOTE") && <div className="courses__payment-row"><Form.Item name="installmentCount" label={paymentPlan === "PROMISSORY_NOTE" ? "Senet adedi" : "Taksit sayısı"} rules={[{ required: true, message: paymentPlan === "PROMISSORY_NOTE" ? "Senet adedini seçin." : "Taksit sayısını seçin." }]}><Select options={Array.from({ length: 23 }, (_, index) => ({ value: index + 2, label: paymentPlan === "PROMISSORY_NOTE" ? `${index + 2} senet` : `${index + 2} taksit` }))} /></Form.Item><Form.Item name="firstPaymentDate" label={paymentDateLabel(paymentPlan)} rules={[{ required: true, message: `${paymentDateLabel(paymentPlan)} seçin.` }]}><DatePicker className="courses__date-picker" format="DD MMMM YYYY" /></Form.Item></div>}
    {paymentPlan === "PROMISSORY_NOTE" && <div className="courses__promissory-fields"><Form.Item name="promissoryDebtorName" label="Senet borçlusu" rules={[{ required: true, message: "Senet borçlusunun adını yazın." }]}><Input maxLength={160} /></Form.Item><Form.Item name="promissoryGuarantorName" label="Kefil"><Input maxLength={160} /></Form.Item><Form.Item name="promissoryIssuePlace" label="Düzenleme yeri" rules={[{ required: true, message: "Senedin düzenleme yerini yazın." }]}><Input maxLength={120} /></Form.Item><Form.Item name="promissorySerialStart" label="Başlangıç senet no"><Input maxLength={80} /></Form.Item></div>}
    <Form.Item name="paymentStatus" label="Ödeme durumu" rules={[{ required: true }]}><Select options={paymentStatusOptions(paymentPlan)} /></Form.Item>
...
function EnrollmentForm({ form, candidates, candidateState, selectedCandidate, remainingCapacity, query, filter, paymentPlan, paymentStatus, onQueryChange, onFilterChange, onRetryCandidates }: { form: ReturnType<typeof Form.useForm<EnrollmentFormValues>>[0]; candidates: Student[]; candidateState: CandidateState["status"]; selectedCandidate?: Student; remainingCapacity: number; query: string; filter: CandidateFilter; paymentPlan?: PaymentPlanType; paymentStatus?: CreateClassEnrollmentRequest["paymentStatus"]; onQueryChange: (value: string) => void; onFilterChange: (value: CandidateFilter) => void; onRetryCandidates: () => void }): JSX.Element {
  return <Form form={form} layout="vertical" requiredMark="optional" className="courses__enrollment-form" initialValues={{ paymentPlan: "CASH", paymentStatus: "PENDING" }}>
    ...
    <section className="courses__payment-form" aria-labelledby="enrollment-payment-title">
      ...
      <Form.Item name="registrationFee" label="Kayıt ücreti" rules={[{ required: true, message: "Kayıt ücretini yazın." }]}><InputNumber className="courses__money-input" min={0} max={9999999999.99} precision={2} prefix="₺" formatter={moneyInputFormatter} parser={moneyInputParser} /></Form.Item>
      <Form.Item name="paymentPlan" label="Ödeme şekli" rules={[{ required: true, message: "Ödeme şeklini seçin." }]}><Segmented block options={paymentPlanOptions} /></Form.Item>
      {(paymentPlan === "INSTALLMENT" || paymentPlan === "PROMISSORY_NOTE") && <div className="courses__payment-row"><Form.Item name="installmentCount" label={paymentPlan === "PROMISSORY_NOTE" ? "Senet adedi" : "Taksit sayısı"} rules={[{ required: true, message: paymentPlan === "PROMISSORY_NOTE" ? "Senet adedini seçin." : "Taksit sayısını seçin." }]}><Select options={Array.from({ length: 23 }, (_, index) => ({ value: index + 2, label: paymentPlan === "PROMISSORY_NOTE" ? `${index + 2} senet` : `${index + 2} taksit` }))} /></Form.Item><Form.Item name="firstPaymentDate" label={paymentDateLabel(paymentPlan)} rules={[{ required: true, message: `${paymentDateLabel(paymentPlan)} seçin.` }]}><DatePicker className="courses__date-picker" format="DD MMMM YYYY" /></Form.Item></div>}
      {paymentPlan === "PROMISSORY_NOTE" && <div className="courses__promissory-fields"><Form.Item name="promissoryDebtorName" label="Senet borçlusu" rules={[{ required: true, message: "Senet borçlusunun adını yazın." }]}><Input maxLength={160} /></Form.Item><Form.Item name="promissoryGuarantorName" label="Kefil"><Input maxLength={160} /></Form.Item><Form.Item name="promissoryIssuePlace" label="Düzenleme yeri" rules={[{ required: true, message: "Senedin düzenleme yerini yazın." }]}><Input maxLength={120} /></Form.Item><Form.Item name="promissorySerialStart" label="Başlangıç senet no"><Input maxLength={80} /></Form.Item></div>}
      <Form.Item name="paymentStatus" label="Ödeme durumu" rules={[{ required: true, message: "Ödeme durumunu seçin." }]}><Select options={paymentStatusOptions(paymentPlan)} /></Form.Item>
      {paymentPlan === "CASH" && paymentStatus === "PENDING" && <Form.Item name="expectedPaymentDate" label="Tahmini ödeme tarihi" rules={[{ required: true, message: "Tahmini ödeme tarihini seçin." }]}><DatePicker className="courses__date-picker" format="DD MMMM YYYY" /></Form.Item>}
...
function CourseCard({ item, can, onEditCourse, onDelete }: { item: Course } & RecordsProps): JSX.Element { return <article className="courses__mobile-card"><Flex justify="space-between" gap={12}><strong>{item.name}</strong><StatusTag value={item.status}/></Flex><Flex justify="space-between"><span>{item.durationHours} saat</span><b>{currency.format(item.listPrice)}</b></Flex><RecordActions canUpdate={can("course:update")} canDelete={can("course:delete")} onEdit={() => onEditCourse(item)} onDelete={() => onDelete("course", item)} /></article>; }
function ClassCard({ item, can, onEditClass, onDelete }: { item: CourseClass } & RecordsProps): JSX.Element { return <article className="courses__mobile-card"><div className="courses__card-main"><Flex justify="space-between" gap={12}><div><strong>{item.name}</strong><small>{item.courseName}</small></div><StatusTag value={item.status}/></Flex><div className="courses__class-meta"><span>{item.instructorName}</span><span>{formatApiDate(item.startDate)} – {formatApiDate(item.endDate)} · {formatClassTime(item)}</span></div><Occupancy item={item}/></div><RecordActions canUpdate={can("class:update")} canDelete={can("class:delete")} onEdit={() => onEditClass(item)} onDelete={() => onDelete("class", item)} /></article>; }
...
function optionalText(value?: string | null): string { return value?.trim() || "-"; }
function formatClassTime(item: Pick<CourseClass, "startTime" | "endTime">): string { return `${item.startTime.slice(0, 5)} - ${item.endTime.slice(0, 5)}`; }
function isScheduledPaymentPlan(paymentPlan?: PaymentPlanType): boolean {
  return paymentPlan === "INSTALLMENT" || paymentPlan === "PROMISSORY_NOTE";
}
function paymentStatusOptions(paymentPlan?: PaymentPlanType): Array<{ value: PaymentStatus; label: string }> {
  const entries = Object.entries(paymentStatusLabels) as Array<[PaymentStatus, string]>;
  return entries
    .filter(([value]) => !isScheduledPaymentPlan(paymentPlan) || value === "PENDING")
    .map(([value, label]) => ({ value, label }));
}
function paymentPlanSummary(student: EnrolledStudent): string {
  if (student.paymentPlan === "PROMISSORY_NOTE") return `${paymentPlanLabels[student.paymentPlan]} · ${student.installmentCount ?? student.payments.length} senet`;
  if (student.paymentPlan === "INSTALLMENT") return `${paymentPlanLabels[student.paymentPlan]} · ${student.installmentCount ?? student.payments.length} taksit`;
  return paymentPlanLabels[student.paymentPlan];
}
function mergeEnrollmentNote(values: EnrollmentFormValues | EnrollmentEditFormValues): string | null {
  const note = values.note?.trim();
  if (values.paymentPlan !== "PROMISSORY_NOTE") return note || null;
  const details = [
    "Senetli kayıt",
    `Borçlu: ${values.promissoryDebtorName?.trim()}`,
    values.promissoryGuarantorName?.trim() ? `Kefil: ${values.promissoryGuarantorName.trim()}` : undefined,
    `Düzenleme yeri: ${values.promissoryIssuePlace?.trim()}`,
    values.promissorySerialStart?.trim() ? `Senet başlangıç no: ${values.promissorySerialStart.trim()}` : undefined,
  ].filter(Boolean).join(" | ");
  return note ? `${details}\n${note}` : details;
}
function paymentDateLabel(paymentPlan: PaymentPlanType): string { return paymentPlan === "PROMISSORY_NOTE" ? "İlk senet vadesi" : "İlk ödeme tarihi"; }
