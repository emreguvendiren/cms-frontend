...
type FormValues = {
  fullName: string; email: string; phone?: string; status: StudentStatus; source: string;
...
export function StudentsWorkspace({ user = { id: "", email: "", authorities: ["student:delete", "student:phone:reveal"] } }: { user?: AuthenticatedUser }): JSX.Element {
  ...
  useEffect(() => {
    let active = true; loadStudents().then((page) => { if (active) setStudents(page.content); })
      .catch(() => message.error("Öğrenci kayıtları yüklenemedi.")).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [message]);
  const filtered = useMemo(() => {
    const q = query.trim().toLocaleLowerCase("tr-TR"); return students.filter((s) =>
      (status === "all" || s.status === status) && (!q || [s.fullName, s.email, s.activeCourse ?? ""].some((v) => v.toLocaleLowerCase("tr-TR").includes(q))));
  ...
  const openEdit = (s: Student) => {
    setEditing(s); form.setFieldsValue({
      fullName: s.fullName, email: s.email, phone: undefined, status: s.status, source: s.source, kvkkConsent: s.kvkkConsent,
      inactiveReason: s.inactiveReason ?? undefined, registrationDate: dayjs(s.registrationDate), expectedStartDate: s.expectedStartDate ? dayjs(s.expectedStartDate) : undefined
    }); setFormOpen(true);
  };
  const save = async () => {
    let values: FormValues; try { values = await form.validateFields(); } catch { return; } setSubmitting(true);
    const common = {
      fullName: values.fullName.trim(), email: values.email.trim(), status: values.status, activeCourse: editing?.activeCourse ?? null,
    ...
    try {
      const saved = editing ? await updateStudent(editing.id, { ...common, phone: values.phone ?? "", version: editing.version }) : await createStudent({ ...common, phone: values.phone ?? "" });
  ...
  const remove = (s: Student) => modal.confirm({
    centered: true, title: `${s.fullName} silinsin mi?`, content: "Kayıt güvenli biçimde pasife alınacak.", okText: "Öğrenciyi sil", okType: "danger", cancelText: "Vazgeç",
